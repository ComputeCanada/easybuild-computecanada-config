...
def patch_rpath(filename, args, interpreter, force_rpath):
...
def patch_zip(filename, args, interpreter, force_rpath):
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        for root, dirs, files in os.walk(os.getcwd()):
            ...
            for name in files:
                ...
                patch_rpath(fname, args, interpreter, force_rpath)
    ...
    for root, dirs, files in os.walk(args.path):
        ...
        for name in files:
            ...
            if name[-4:] in {".whl", ".jar"}:
                patch_zip(f, args, interpreter, force_rpath)
            else:
                patch_rpath(f, args, interpreter, force_rpath)
    return 0
